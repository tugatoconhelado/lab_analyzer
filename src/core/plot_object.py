import os
import sys
from src.core.structures import LineConfig, AxesConfig, WorkbenchAsset, Trace
...
class PlotObject(WorkbenchAsset):
    ...
    """
    def __init__(self, plot_name, registry, asset_id=None):
    ...
    def add_trace(self, trace_id: int):
        if trace_id not in self.trace_ids:
            if not self.verify_trace_id(trace_id):
                return
            self.trace_ids.append(trace_id)
            self.trace_configs[trace_id] = LineConfig(label=str(trace_id))
            trace = self.get_data(trace_id)
